...
public sealed class MySqlQueryDescriberErrorTests(MySqlFixture fixture)
{
    [Fact]
    ...
    public async Task Should_ThrowArgumentException_When_CommandTextIsNullOrWhiteSpace(string? commandText)
    {
        ...
        await Assert.ThrowsAnyAsync<ArgumentException>(
            () => new MySqlQueryDescriber().DescribeAsync(connection, commandText!, TestContext.Current.CancellationToken));
    }
    ...
    public async Task Should_ThrowArgumentNullException_When_ConnectionIsNull()
    {
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => new MySqlQueryDescriber().DescribeAsync(null!, "SELECT 1", TestContext.Current.CancellationToken));
    }
}
